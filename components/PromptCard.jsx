...
export default function PromptCard({post , handleTagClick , handelEdit , handelDelete}) {
    ...
    <div  className='prompt_card'>
      ...
      { session?.user.id === post.creator._id && pathName === "/profile" && (
        <div className='mt-5 flex-center gap-4 border-t border-gray-100 pt-3'>
          <p className='font-inter text-sm green_gradient cursor-pointer' onClick={handelEdit}>Edit</p>
          
          <p className='font-inter text-sm orange_gradient cursor-pointer' onClick={handelDelete}>Delete</p>
        </div>
      ) }
    </div>
  )
